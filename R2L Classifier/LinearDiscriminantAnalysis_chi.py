...
X_R2L=newdf.drop('label',1)
Y_R2L=newdf.label
X_R2L_test = newdf_test.drop('label',1)
Y_R2L_test = newdf_test.label

colNames=list(X_R2L)

from sklearn.feature_selection import SelectKBest 
from sklearn.feature_selection import chi2
chi2f = SelectKBest(chi2, k = 54)#iterate the k from 1 to 120. The max. accuracy comes at k=54 .
chi2f.fit(X_R2L , Y_R2L)
true=chi2f.get_support()
chicolindex_R2L=[i for i, x in enumerate(true) if x]
chicolname_R2L=list(colNames[i] for i in chicolindex_R2L)
print('Features selected :',chicolname_R2L)

features = newdf[chicolname_R2L].astype(float)
features1 = newdf_test[chicolname_R2L].astype(float)
lab = newdf['label']
...
#Features selected : ['duration', 'src_bytes', 'dst_bytes', 'wrong_fragment', 'urgent', 'hot', 'num_failed_logins', 'logged_in', 'num_compromised', 'root_shell', 'num_root', 'num_file_creations', 'num_shells', 'num_access_files', 'is_guest_login', 'count', 'srv_count', 'serror_rate', 'srv_serror_rate', 'rerror_rate', 'srv_rerror_rate', 'same_srv_rate', 'diff_srv_rate', 'srv_diff_host_rate', 'dst_host_count', 'dst_host_srv_count', 'dst_host_same_srv_rate', 'dst_host_diff_srv_rate', 'dst_host_same_src_port_rate', 'dst_host_srv_diff_host_rate', 'dst_host_serror_rate', 'dst_host_srv_serror_rate', 'dst_host_rerror_rate', 'dst_host_srv_rerror_rate', 'Protocol_type_icmp', 'Protocol_type_tcp', 'Protocol_type_udp', 'service_domain_u', 'service_eco_i', 'service_ecr_i', 'service_ftp', 'service_ftp_data', 'service_http', 'service_imap4', 'service_other', 'service_private', 'service_smtp', 'service_telnet', 'flag_REJ', 'flag_RSTO', 'flag_S0', 'flag_S3', 'flag_SF', 'flag_SH']
#Classifir trained in 1.816 seconds
