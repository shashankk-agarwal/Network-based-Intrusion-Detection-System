#FEATURE_SELECTION ALGORITHM : VarianceThreshold
import pandas as pd
import numpy as np
...
variance_threshold.fit(X_DOS)
true=variance_threshold.get_support()
varcolindex_DOS=[i for i, x in enumerate(true) if x]
varcolname_DOS=list(colNames[i] for i in varcolindex_DOS)
print('Features selected :',varcolname_DOS)

features = newdf[varcolname_DOS].astype(float)
features1 = newdf_test[varcolname_DOS].astype(float)
lab = newdf['label']
...
#Classifier trained in 17.564 seconds.
#Predicted in 21.577 seconds
#Accuracy is 0.9646.
